...
export const Projects = ()=>{
    return (
        <div className=" p-8 space-y-3 text-[#6b6b68]">
            ...
            <div className="grid grid-cols-2 gap-4">{projects.map((data)=><ProjectCard key={data.title} title={data.title} description={data.description} githubSrc={data.githubSrc}/>)}</div>
        </div>
    )
}
...
const ProjectCard = ({name,title,description,githubSrc})=>{
    return (
        <div key={name} className="bg-black border p-4 rounded-2xl space-y-1">
