    ...
    <div className=" p-8 space-y-2">
      ...
      <div className="flex flex-col items-center mt-3 space-y-2">
        ...
        <a href="../../../Resume.pdf" target="_blank" rel="noopener noreferrer">
          <span className="inline-block cursor-pointer px-3 py-1 border border-gray-500 rounded-xl hover:text-white hover:bg-black  transition-all transform hover:scale-110 duration-300 ">
            View Resume
          </span>
        </a>
      </div>
...
const InlineSkillsSpan = ({ data }) => {
  return (
    <div className="flex flex-wrap gap-2">
      {data.map((skill, ind) => (
        <span key={ind} className="border border-gray-500 p-1 rounded-xl">
          {skill}
        </span>
      ))}
    </div>
